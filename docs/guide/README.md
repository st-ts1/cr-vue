...
大抵のエラーはこのメッセージがヒントとなり、エラーの理由を知ることができます。
...
`... undefined` というエラーは「○○が定義されていません」といった内容ですが、実は本当にこの通りで「算出プロパティを`methods`の中に書いてしまっていた」「消したプロパティをまだテンプレートで使っていた」というようなミスもありがちです。
